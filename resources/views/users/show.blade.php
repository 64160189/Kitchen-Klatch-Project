    ...
    <div class="col-6">
        ...
        <div class="content">
            ...
            <div class="btn-group mb-3" role="group" aria-label="Post View Type">
                ...
                <button type="button" class="btn btn-secondary" id="shared-posts" title="แสดงโพสต์ที่แชร์">โพสต์ที่แชร์</button>
            </div>

            <div id="post-container">
                @foreach ($posts as $item)
                    <div class="post-frame card mb-4 shadow bg-secondary-subtle" onclick="goToPost({{ $item->id }})">
                        <img class="card-img-top" src="{{ asset('storage/' . $item->image) }}" alt="{{ $item->title }}" style="max-height: 800px;">
                        <div class="card-body">
                            <h2 class="card-title">{{ $item->title }}</h2>
                            <p class="card-text">{{ Str::limit($item->description, 50) }}</p>
                            <h3 class="card-text">วัตถุดิบ:</h3>
                            <p class="card-text">{{ Str::limit(is_array($item->ingrediant) ? implode(', ', $item->ingrediant) : $item->ingrediant, 50) }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            @if ($posts->hasMorePages())
                <div class="d-flex justify-content-center">
                    <button id="load-more" class="btn btn-danger" data-page="{{ $posts->currentPage() + 1 }}" data-user="{{ $user->id }}" title="โหลดโพสต์เพิ่มเติม">
                        โหลดเพิ่มเติม
                    </button>
                </div>
            @endif
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadPosts() {
        ...
        .then(data => {
            ...
            if (currentPage === 1) postContainer.innerHTML = ''; // Clear old posts if it's the first page
